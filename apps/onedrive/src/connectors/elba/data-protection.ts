...
import { type OnedrivePermission } from '../microsoft/onedrive/permissions';
...
const formatDataProtectionPermissions = (permissions: OnedrivePermission[]) => {
  ...
  for (const { item, permissions } of items) {
    ...
    if (formattedPermissions.length) {
      const object = {
        ...
        metadata: { userId } satisfies ObjectMetadata,
