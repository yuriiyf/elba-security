...
describe('auth connector', () => {
  describe('getToken', () => {
    beforeEach(() => {
      server.use(
        http.post(`${env.FRONT_APP_INSTALL_URL}/token`, async ({ request }) => {
    ...
    test('should throw when the code is invalid', async () => {
      await expect(getToken('wrong-code')).rejects.toBeInstanceOf(FrontError);
    });
  });
  ...
  describe('getRefreshToken', () => {
    ...
    test('should throw when the refreshToken is invalid', async () => {
      await expect(getRefreshToken('wrong-refreshtoken')).rejects.toBeInstanceOf(FrontError);
    });
  });
});
