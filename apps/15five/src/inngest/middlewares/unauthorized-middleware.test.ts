...
describe('unauthorized middleware', () => {
  ...
  test('should not transform the output when the error is not about fifteenFive authorization', async () => {
    const send = vi.fn().mockResolvedValue(undefined);
    await expect(
      unauthorizedMiddleware
        // @ts-expect-error -- this is a mock
  ...
  test('should transform the output error to NonRetriableError and remove the organisation when the error is about fifteenFive authorization', async () => {
    const unauthorizedError = new FifteenFiveError('foo bar', {
      // @ts-expect-error this is a mock
      response: {
        status: 403,
      },
    ...
    expect(send).toBeCalledWith({
      name: 'fifteenfive/app.uninstalled',
      data: {
        organisationId: organisation.id,
      },
    });
