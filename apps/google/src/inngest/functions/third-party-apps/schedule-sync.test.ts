...
const setup = createInngestFunctionMock(scheduleThirdPartyAppsSync);
...
describe('schedule-third-party-apps-sync', () => {
  ...
  test('should not start third party apps sync when there are no organisations', async () => {
  ...
  test('should start third party apps sync successfully', async () => {
    ...
    expect(step.sendEvent).toBeCalledWith('start-third-party-apps-sync', [
      {
        data: {
          isFirstSync: false,
